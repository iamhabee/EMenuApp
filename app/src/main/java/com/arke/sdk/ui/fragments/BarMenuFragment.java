...
public class BarMenuFragment extends BaseFragment {
    ...
    private void handleIncomingEvent(Object event) {
        if (event instanceof ItemSearchEvent) {
            ...
            if (eMenuItems.contains(deletedItem)) {
                ...
                checkAndDisplayEmptyView();
                if (eMenuItems.isEmpty()) {
                    CollectionsCache.getInstance().clearCache(Globals.BAR_CONTENTS_CACHE);
                }
            }
...
//                swipeRefreshLayout.setRefreshing(false);
    ...
    private void checkAndDisplayEmptyView() {
        if (eMenuItems.isEmpty()) {
    ...
    private void displayTapTargetMaterialPrompt() {
        if (getActivity() != null) {
            new MaterialTapTargetPrompt.Builder(getActivity())
                    .setTarget(BarHomeActivity.addNewMenuItem)
                    .setPrimaryText("Setup Your EMenu Bar")
                    .setSecondaryText("Your Bar Emenu seems to be empty. Tap here to setup drinks for your Bar.")
...
//        swipeRefreshLayout.setRefreshing(false);
        ...
        if (!eMenuItems.isEmpty()) {
            CollectionsCache.getInstance().cacheEMenuItems(Globals.BAR_CONTENTS_CACHE, eMenuItems);
        }
    }
...
//                ContextCompat.getColor(getActivity(), R.color.gplus_color_4));
        ...
        contentRecyclerView.addOnScrollListener(new HideShowScrollListener() {
            @Override
            public void onHide() {
                BarHomeActivity.addNewMenuItem.hide();
            }

            @Override
            public void onShow() {
                BarHomeActivity.addNewMenuItem.show();
            }
        });
    }
